...
interface IStrategy {
  ...
  event WrappedNativeChanged(address indexed oldWrappedNative, address indexed newWrappedNative);
  ...
  event LpRouterChanged(address indexed oldLpRouter, address indexed newLpRouter);
  ...
  function getHarvestable() external view returns (address[] memory rewards, uint256[] memory amounts);
