...
contract SteerZapper is ZapperBase {
    ...
    returns (uint256 tokenOutAmount, ReturnedAsset[] memory returnedAssets)
  {
    ...
    tokenOutAmount = lpRouter.addLiquidity(asset, tokenIn, tokenInAmount, address(this), IDexType.DexType.STEER);
  ...
  function swapFromAssets(
    ...
    returns (uint256 tokenOutAmount, ReturnedAsset[] memory)
  {
    if (recipient != address(this)) {
    ...
    tokenOutAmount = lpRouter.removeLiquidity(asset, assetsInAmount, address(this), tokenOut, IDexType.DexType.STEER);
