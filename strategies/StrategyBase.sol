...
abstract contract StrategyBase is SphereXProtected, ReentrancyGuard, IStrategy {
  ...
  receive() external payable {
    if (msg.sender != address(bgt) && msg.sender != wrappedNative && msg.sender != address(zapper))
  ...
  function _revertOnlyBenevolent() internal view {
    if (!harvesters[msg.sender] && msg.sender != governance && msg.sender != strategist) revert NotBenevolent();
  }

  /// @notice Claim BGT, convert to BERA and swap to asset using zapper
  /// @return amount Amount of asset received
  function _swapBGTToAsset() internal sphereXGuardInternal(0x6e7957c8) returns (uint256 amount) {
    ...
    if (balance > 0) {
      ...
      (amount, ) = zapper.swapToAssets(address(asset), address(wrappedNative), beraBalance, address(this));
    }
  }
  ...
  function setWithdrawalDevFundFee(uint16 fee) external onlyTimelock sphereXGuardExternal(0x219c2d6e) {
    if (fee + withdrawalTreasuryFee > MAX_WITHDRAWAL_FEE) revert FeeTooHigh(fee, MAX_WITHDRAWAL_FEE);
  ...
  function setWithdrawalTreasuryFee(uint16 fee) external onlyTimelock sphereXGuardExternal(0x63c66135) {
    if (fee + withdrawalDevFundFee > MAX_WITHDRAWAL_FEE) revert FeeTooHigh(fee, MAX_WITHDRAWAL_FEE);
  ...
  /// @param fee New fee in basis points
  function setPerformanceDevFee(uint16 fee) external onlyTimelock sphereXGuardExternal(0x39189706) {
    if (fee + performanceTreasuryFee > MAX_PERFORMANCE_FEE) revert FeeTooHigh(fee, MAX_PERFORMANCE_FEE);
  ...
  /// @param fee New fee in basis points
  function setPerformanceTreasuryFee(uint16 fee) external onlyTimelock sphereXGuardExternal(0x76672d92) {
    if (fee + performanceDevFee > MAX_PERFORMANCE_FEE) revert FeeTooHigh(fee, MAX_PERFORMANCE_FEE);
  ...
  function setStrategist(address strategistAddress) external onlyGovernance sphereXGuardExternal(0xfbe05f47) {
  ...
  function setGovernance(address governanceAddress) external onlyGovernance sphereXGuardExternal(0x1bb893a5) {
  ...
  function setTimelock(address timelockAddress) external onlyTimelock sphereXGuardExternal(0xee3d9cc1) {
  ...
  function setController(address controllerAddress) external onlyTimelock sphereXGuardExternal(0x4d05853d) {
  ...
  function setSwapRouter(address swapRouterAddress) external onlyGovernance sphereXGuardExternal(0xa8988767) {
  ...
  function setLpRouter(address lpRouterAddress) external onlyGovernance sphereXGuardExternal(0x5c5c6eff) {
  ...
  function setZapper(address zapperAddress) external onlyGovernance sphereXGuardExternal(0x4d0d76be) {
  ...
  function getHarvestable() external view virtual returns (address[] memory rewards, uint256[] memory amounts) {
  ...
  /// @dev Must be implemented by specific strategy implementations
  function harvest() public virtual nonReentrant onlyBenevolent sphereXGuardPublic(0x8ab9334c, 0x4641257d) {
  ...
  function withdraw(
    address token
  ) external nonReentrant onlyController sphereXGuardExternal(0xf1588fd0) returns (uint256 balance) {
  ...
  /// @param amount Amount of assets to withdraw
  function withdraw(uint256 amount) external nonReentrant onlyController sphereXGuardExternal(0xbc6047d4) {
    ...
    if (feeDev > 0) {
      IERC20(asset).safeTransfer(IController(controller).devfund(), feeDev);
      emit WithdrawalDevFundFeeCollected(feeDev);
    }

    uint256 feeTreasury = (amount * withdrawalTreasuryFee) / MAX_FEE_BPS;
    if (feeTreasury > 0) {
      IERC20(asset).safeTransfer(IController(controller).treasury(), feeTreasury);
  ...
  function withdrawForSwap(
    uint256 amount
  ) external nonReentrant onlyController sphereXGuardExternal(0x8f20ccb0) returns (uint256 balance) {
  ...
  function execute(
    address target,
    bytes memory data
  ) public payable onlyTimelock sphereXGuardPublic(0x3acab6ad, 0x1cff79cd) returns (bytes memory response) {
  ...
  function _distributePerformanceFeesBasedAmountAndDeposit(uint256 amount) internal sphereXGuardInternal(0xff626c51) {
    ...
    if (amount > 0) {
      // Treasury fees
      uint256 treasuryFee = (amount * performanceTreasuryFee) / MAX_FEE_BPS;
      if (treasuryFee > 0) {
        IERC20(asset).safeTransfer(IController(controller).treasury(), treasuryFee);
      ...
      if (devFee > 0) {
        IERC20(asset).safeTransfer(IController(controller).devfund(), devFee);
