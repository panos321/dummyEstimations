...
/**
 * @title BurrBearStrategy
 * @notice A strategy contract for managing BurrBear LP positions
 * @dev This strategy is a basic implementation that holds BurrBear LP tokens without additional staking
 * It inherits from StrategyBase and overrides the required functions
 */
contract BurrBearStrategy is StrategyBase {
  /**
   * @notice Initializes the BurrBear strategy
  ...
  function deposit() public override sphereXGuardPublic(0xc1070366, 0xd0e30db0) {
  ...
  function _withdrawSome(uint256 amount) internal virtual override sphereXGuardInternal(0x0f4c2f3b) returns (uint256) {
